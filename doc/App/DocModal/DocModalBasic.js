		...
		return (
			<div className="DocModal">
				...
				{this.generateModal('modal1', {
					onRequestClose: e => this.setState({ modal1: false }),
				})}

				{this.generateModal('modal2a', { 
					className: '__is-scrollable',
					onRequestClose: e => this.setState({ modal2a: false }),
				})}

				{this.generateModal('modal2b', { 
					className: '__sm',
					onRequestClose: e => this.setState({ modal2b: false }),
				})}

				{this.generateModal('modal2c', { 
					className: '__lg',
					onRequestClose: e => this.setState({ modal2c: false }),
				})}

				{this.generateModal('modal3', { 
					style: { width: '400px' },
					bodyClassName: 'modal3__opened',
					onOpen: e => alert('Modal is opened!'),
					onClose: e => alert('Modal is closed!'),
				})}
			</div>
		)
	}
	...
	renderSample() {
		return (
			<div className="doc-modal">
				...
				<div className="group">
					<div className="grid-12 grid-p-sm-15">
						<div className="c-sm-4">
							<Button theme="blue" onClick={e => this.setState({ modal2a: true })}>
								Show Scrollable Modal
							</Button>
						</div>
						<div className="c-sm-4">
							<Button theme="green" onClick={e => this.setState({ modal2b: true })}>
								Show SM Modal
							</Button>
						</div>
						<div className="c-sm-4">
							<Button theme="red" onClick={e => this.setState({ modal2c: true })}>
								Show LG Modal
							</Button>
						</div>
					</div>
				...
				<div className="group">
					<Button theme="ember" onClick={e => this.setState({ modal3: true })}>
						Show Modal
					</Button>
				</div>
			</div>
		...
		return (
			...
			>
				...
				<div className="Modal-footer">
					<FlatButton theme="grey" onClick={e => this.setState({ [modalId]: false })}>
						Cancel
					</FlatButton>
					<FlatButton theme="green" onClick={e => this.setState({ [modalId]: false })}>
						Got it!
					</FlatButton>
				</div>
			</Modal>
